...
const Cards = ({data, reference}) => {
  return (
    <motion.div drag dragConstraints={reference} whileDrag={{scale: 1.2}} dragTransition={{ bounceStiffness: 600, bounceDamping: 10 }}
      ...
      <FaRegFileAlt />
      <p className="mt-5 text-sm leading-tight font-sans">
        {data.desc}
      </p>
      ...
      <div className="footer absolute w-full left-0 bottom-0">
        ...
        {
          data.tag.isOpen && (
            <div className={`tag w-full h-12 ${data.tag.tagColor === 'sky' ? "bg-sky-500" : "bg-green-500"} flex items-center justify-center`}>
